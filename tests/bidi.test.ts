...
describe("firstStrongDir", () => {
  it("returns 'rtl' for pure Persian", () => {
    expect(firstStrongDir("سلام دنیا")).toBe("rtl");
  });
  ...
  it("returns 'rtl' for mixed Persian + English", () => {
    expect(firstStrongDir("سلام Hello")).toBe("rtl");
    expect(firstStrongDir("Hello سلام")).toBe("rtl");
  });

  it("returns 'rtl' when RLM is present", () => {
    expect(firstStrongDir(RLM + "test")).toBe("rtl");
  });

  it("returns 'ltr' when LRM is present alone", () => {
    expect(firstStrongDir(LRM + "123")).toBe("ltr");
  });

  it("returns null for neutral-only text", () => {
    expect(firstStrongDir("123 456")).toBe(null);
  });
